...
{
    public:
        CRobot(JNIEnv* p_env) : p_env(p_env)
        {
            Init();
        }
        ...
        bool Init()
        {
            ...
            if (class_ptr == nullptr) {
                printf("[-] Failed to find %s class pointer\n", name);
                return false;
            }
            ...
            if (class_instance == nullptr) {
                printf("[-] Failed to find %s class instance\n", name);
                return false;
            }
        }
        ...
        void delay(jint ms) {
            static jmethodID method_id = p_env->GetMethodID(class_ptr, "ms", "(I)V");
